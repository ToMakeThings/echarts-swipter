...
(function(root, factory) {
	...
	Mobilebone.currentPage;
	Mobilebone.previousPage;
	
	Mobilebone.currentUrl;
    Mobilebone.previousUrl;
	
	Mobilebone.iscrollStore = {};
		...
		if (pageInto != null && pageInto.classList) {		
            ...
            if (index > 0) {
                if (options.pageParam) {
                    options.pageParam = options.pageParam + "&" + pageid.substr(index+1);
                } else {
                    options.pageParam = pageid.substr(index+1);
                }  
                if (pageid.toLowerCase().indexOf("cjcp.com.cn") < 0) {
                    pageid = pageid.substr(0, index);
                }
            }

			if (options.remove !== false && Mobilebone.store[pageid] && Mobilebone.store[pageid] != pageInto && Mobilebone.store[pageid].parentElement) {
				Mobilebone.store[pageid].parentElement.removeChild(Mobilebone.store[pageid]);
				delete Mobilebone.store[pageid];
			}
			...
			var attr_reload = pageInto.getAttribute("data-reload") || options.needReload;
			// add parent page's params
			if (options.pageParam) {
			    pageInto.setAttribute("trans-params", options.pageParam);
			} else if (pageInto.getAttribute("trans-params") && back == false) {
			    pageInto.removeAttribute("trans-params");
			}
			
			// do callback when come in first time
			var onpagefirstinto = params_in.onpagefirstinto;
			if (!Mobilebone.store[pageid]) {
			...
			//alert(supportHistory + ", " + this.pushStateEnabled + ", " + options.history + ","+url_push);
			...
			if (!Mobilebone.store[pageid]) {
				Mobilebone.store[pageid] = pageInto;
			}
			
			var attr_mask = pageInto.getAttribute("data-mask");
            if (attr_mask == "true" || attr_mask == "") {
                ele_mask = pageInto.querySelector("." + this.classMask);
            }
            
            // current show page
            Mobilebone.currentPage = pageInto;
            if (pageOut) {
                Mobilebone.previousPage = pageOut;
            }
            ...
            if (ele_mask == null) {
                ...
                ele_mask.id = "pageLoading";
                ele_mask.className = this.classMask;
                ele_mask.innerHTML = '<div class="loading"></div>';
                if (typeof attr_mask == "string") {
                    pageInto.appendChild(ele_mask);
                } else {
			...
			// do callback
			var iscroll = Mobilebone.iscrollStore[pageInto.id];
            if (iscroll) {
                //iscroll.scrollToElement(pageinto.querySelector(".content"));
                iscroll.scrollTo(0,0);
            }
			if (!(back == true && (attr_reload == null || attr_reload == "false"))) {
    			var callback = params_in.callback;
    			if (typeof callback == "string") callback = params_in.root[callback];
    			if (typeof callback == "function") 
    			    callback(pageInto, pageOut, options.response, ele_mask);
    			refreshIScroll();
			}
		}
	};
	...
	**/
	 ...
	 *
	**/
	Mobilebone.ajax = function(trigger_or_options, clean_param) {
        ...
        if (clean_param && clean_param != "") {
            params.pageParam = clean_param;
        }
		...
		if (ele_mask == null) {
			ele_mask = document.createElement("div");
			ele_mask.id = "ajaxLoading";
			ele_mask.className = this.classMask;
			ele_mask.innerHTML = '<div class="load_box"><div class="loading"></div><div class="load_txt">加载中... </div></div>';
			if (typeof attr_mask == "string") {
		...
		xhr.onload = function() {
			...
			if (xhr.status == 200) {
				...
				if (params.dataType == "json" || params.dataType == "JSON") {
					try {
						response = JSON.parse(xhr.response ? xhr.response : xhr.responseText);
	...
	**/
    ...
    render = function(page_in, page_out, json) {
	...
	**/
	Mobilebone.init = function() {	
		...
		if (hash == "" || hash == "#") {
		    ...
		    var urlInit = hash.replace("#", "");
            Mobilebone.currentUrl = urlInit;
			// as a ajax
			this.ajax({
				url: urlInit,
				dataType: "unknown",
	...
	**/
	Mobilebone.handleTapEvent = function(event) {
		...
		if (/^#/.test(target.getAttribute("href")) == true) {
			...
			Mobilebone.previousUrl = Mobilebone.currentUrl;
			Mobilebone.currentUrl = clean_url;
            
            var clean_param = "";
            
            if (clean_url.toLowerCase().indexOf("cjcp.com.cn") < 0) {
                var index = clean_url.indexOf("?");
                if (index > 0) {
                    clean_param = (clean_url.substr(index + 1, clean_url.length - index - 1));
                    clean_url = clean_url.substr(0, index);
                }
            }
			...
			if (Mobilebone.store[clean_url]) {
				if (back == false && rel == "auto") {
					back = Mobilebone.isBack(Mobilebone.store[clean_url], self_page);
				}
				
				if (clean_param != "") {
				    Mobilebone.transition(Mobilebone.store[clean_url], self_page, back, {
                        id: clean_url,
                        needReload: attr_reload,
                        pageParam: clean_param
                    });
				} else {
				    Mobilebone.transition(Mobilebone.store[clean_url], self_page, back, {
                        id: clean_url,
				...
				Mobilebone.ajax(target, clean_param);
			}
			event.preventDefault();	
		}
	};
	...
	**/
	window.addEventListener("popstate", function() {
		...
		var page_in = Mobilebone.store[hash];
